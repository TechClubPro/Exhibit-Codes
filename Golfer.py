...
while True:
    try:
        if state=="Start":
            ...
            print("Hit the ball at bin ",_bin)
        ...
        if state=="SetGolfer":
            while final=='n':
                position=int(input("Please set the Angle: "))
                print(position)
                pyro.MoveServo(9,position)
                final=input("Do you want to hit at "+str(position)+": y/n")
                if final=='y':
                  pyro.MoveServo(10,179)
                  time.sleep(3)
